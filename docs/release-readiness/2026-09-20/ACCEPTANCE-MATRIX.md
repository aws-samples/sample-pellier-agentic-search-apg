...
| I06 | Automated root bootstrap, partial failure, rollback and teardown | BLOCKED | Authorized isolated Isengard root rehearsal is being prepared with a deployment suffix and separate Aurora/Cognito. Existing shared resources must remain unchanged; lifecycle proof is not yet complete. |
...
| I08 | Domain-independent origin transport and private exposure boundary | BLOCKED | Original public HTTP origin is being replaced with a private VPC origin/internal ALB and private editor. Viewer HTTPS uses the AWS-managed CloudFront hostname; private hops remain HTTP, not end-to-end TLS. Root/SSE/WebSocket/replacement proof is required before PASS. No shared private key or participant DNS/ACM setup. |
