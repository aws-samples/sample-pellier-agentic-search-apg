...
test('separate storefront tour preserves the hero and exposes all three recorded examples', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1 })).toContainText('from question to evidence')
  ...
  await loop.getByRole('button', { name: 'View all three' }).click()
...
for (const viewport of [{ width: 1440, height: 900 }, { width: 1280, height: 720 }, { width: 768, height: 1024 }, { width: 390, height: 844 }]) {
  test(`tour and chat stay usable at ${viewport.width}x${viewport.height}`, async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'View all three' }).click()
...
test('technical details follow the persona, SQL source, and matching exercise', async ({ page }) => {
  ...
  await loop.locator('[data-step-id="catalog"] .resolution-trace-step-head').click()
  await expect(loop.getByLabel('Source SQL excerpt · vector lookup')).toContainText('embedding <=>')
  ...
  await loop.getByRole('button', { name: 'Bring in a person', exact: true }).click()
  ...
  await loop.locator('[data-step-id="policy"] .resolution-trace-step-head').click()
  await expect(loop.getByLabel('Recorded policy receipt · public fields')).toContainText('"decision": "DENY"')
