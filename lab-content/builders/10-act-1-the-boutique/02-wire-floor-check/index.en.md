...
You'll replace one stubbed tool body – `floor_check` – between the `START` / `END` markers, then verify Marco's Turn 4 lands against live Aurora warehouse data. **⏩ Out of time?** A one-line `cp` from `solutions/closing-marcos-gap/` swaps in the reference implementation – the act still completes.
...
3. Verify the wiring **two ways**: the Atelier's tool registry strip
   (12/13 → 13/13 shipped) and a replay of Marco's Turn 4 in the
...
`floor_check` is a deterministic read against an operational source of truth. In Pellier that source is warehouse inventory. In another stack the same pattern reads bed capacity, eligibility status, claim status, account standing, parts availability, shipment state, or asset telemetry. The decorator declares the contract; the body decides which system of record answers.
:::
...
pill. The progress strip read **12/13 shipped**.

After your save, hard-reload the Atelier tab. The strip should now
read **13/13 shipped** and `floor_check` has a solid border with a
sage *Shipped* pill.
...
::::expand{header="If the strip still says 12/13"}
...
This time Stock Keeper's reply names Brooklyn (`BK-01`), the quantity, the ship window, and the other warehouses' counts. The trace chips read `floor_check` with a real duration. **The stub envelope is gone, and that is the gap closed.**
...
- **Closed Marco's Turn 4.** Turns 1–3 (`find_pieces`, `style_match`,
  `price_intelligence`) and Turn 5 (`escalate_to_stylist`) already
  worked; this change isolated and fixed the stock-specific path.
- **Bumped the Atelier's tool inventory** from 12 shipped + 1 exercise
  to 13 shipped – in real time, without restarting anything.
...
:::alert{type="success" header="Anna's anchor next – prove rerank earns its cost"}
Turn 4 lands. Switch to Anna and decide whether the agentic
retrieval path (Haiku extract → filtered vector → rerank) earns its
latency over plain vector or hybrid (RRF). The mandatory build
moment in Act II is **Exercise 2** – one `SELECT` against
`pellier.tool_audit` to recover a Cedar-gated mutation; an optional
skill-edit lives at the bottom of the next page if your table has
time.
