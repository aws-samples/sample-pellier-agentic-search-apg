...
rows. **MCP** lets the same tools work from the IDE sidebar; **AgentCore
Memory** keeps the conversation in order; **AgentCore Runtime** moves
...
| **Act III · The Concierge** | Read the dispatcher + specialists pattern in the Atelier, then [open Aurora MCP and compare to Bedrock Knowledge Bases](/30-act-3-the-concierge/02-mcp-and-knowledge-bases/) | ~7 min |
...
| **MCP** | Aurora MCP server runs in the IDE sidebar; the same `pellier.product_catalog` and `pellier.warehouse_inventory` tables reachable through MCP tool-calls — see [Act III · MCP and Knowledge Bases](/30-act-3-the-concierge/02-mcp-and-knowledge-bases/) |
