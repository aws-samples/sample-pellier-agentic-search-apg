...
SELECT jsonb_build_object(
 ...
 'sourceTurnExists', EXISTS (SELECT 1 FROM pellier.messages m JOIN review a
   ON m.metadata->>'turn_id'=a.source_turn_id
...
def assess(observed: dict, *, phase: str, source_turn: str, baseline: dict | None = None) -> dict:
    ...
    checks = {
        ...
        'exactSourceTurn': bool(source_turn) and review.get('source_turn_id') == source_turn
            and observed.get('sourceTurnExists') is True,
    ...
    return {'phase': phase, 'observed': observed, 'assertions': checks,
...
def main() -> int:
    ...
    if args.review_id < 1 or not args.source_turn.startswith('turn-'):
        parser.error('Copy the review ID and originating turn from this Operator proposal')
    ...
    print(json.dumps({'phase': args.phase, 'passed': report['passed'], 'assertions': report['assertions']}, indent=2))
